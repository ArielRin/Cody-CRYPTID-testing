...
contract AAA25test is Context, IERC20, Ownable {
    ...
    constructor(address routerAddress) {
        ...
        _isExcludedFromFee[nftRewardWallet] = true;
    ...
    function _takeNftReward(uint256 rNftReward, uint256 tNftReward) private {
        totFeesPaid.nftReward += tNftReward;

        if (_isExcluded[address(this)]) {
            _tOwned[address(this)] += tNftReward;
        }
        _rOwned[address(this)] += rNftReward;
    }
    ...
    function _getTValues(
        ...
        s.tNftReward = (tAmount * temp.nftReward) / 100;
        ...
        )
    {
        ...
        rNftReward = s.tNftReward * currentRate;
        ...
        rTransferAmount =
            ...
            rDev -
            rOps;
        return (rAmount, rTransferAmount, rRfi, rNftReward, rLiquidity);
    }
    ...
    function _tokenTransfer(
        ...
        if (s.rLiquidity > 0 || s.tLiquidity > 0) {
            ...
            emit Transfer(
                sender,
                address(this),
                s.tLiquidity + s.tNftReward + s.tDev + s.tOps
            );
        }
        if (s.rNftReward > 0 || s.tNftReward > 0) _takeNftReward(s.rNftReward, s.tNftReward);
    ...
    function swapAndLiquify(uint256 contractBalance, Taxes memory temp) private lockTheSwap {
        ...
        swapTokensForPLS(toSwap);

        uint256 deltaBalance = address(this).balance - initialBalance;
        uint256 unitBalance = deltaBalance / (denominator - temp.liquidity);
        uint256 plsToAddLiquidityWith = unitBalance * temp.liquidity;

        if (plsToAddLiquidityWith > 0) {
            // Add liquidity to pancake
            addLiquidity(tokensToAddLiquidityWith, plsToAddLiquidityWith);
        }

        uint256 nftRewardAmt = unitBalance * 2 * temp.nftReward;
        if (nftRewardAmt > 0) {
            payable(nftRewardWallet).sendValue(nftRewardAmt);
        }
    ...
    function addLiquidity(uint256 tokenAmount, uint256 plsAmount) private {
        ...
        // add the liquidity
        router.addLiquidityETH{ value: plsAmount }(
            address(this),
            tokenAmount,
            0, // sozz slippage is unavoidable
            0, // sozz slippage is unavoidable
            deadWallet,
            block.timestamp
        );
    ...
    function burn(uint256 _value) public onlyOwner {
        require(_value <= _rOwned[msg.sender], "Insufficient balance to burn");
        _rOwned[msg.sender] = _rOwned[msg.sender] - _value;
    ...
    function updateNftRewardWallet(address newWallet) external onlyOwner {
        require(newWallet != address(0),"Fee Address cannot be zero address");
        nftRewardWallet = newWallet;
    }
    ...
    // Function to update buy taxes
    function setBuyTaxes(uint256 rfi, uint256 nftReward, uint256 ops, uint256 liquidity, uint256 dev) external onlyOwner {
        taxes.rfi = rfi;
        taxes.nftReward = nftReward;
        taxes.ops = ops;
    ...
    // Function to update sell taxes
    function setSellTaxes(uint256 rfi, uint256 nftReward, uint256 ops, uint256 liquidity, uint256 dev) external onlyOwner {
        sellTaxes.rfi = rfi;
        sellTaxes.nftReward = nftReward;
        sellTaxes.ops = ops;
    ...
    function rescuePLS(uint256 weiAmount) external onlyOwner {
        require(address(this).balance >= weiAmount, "insufficient PLS balance");
